...
<!-- END DOUYIN -->"""
    ...
    def run(self):
        ...
        print(cleaned_data)
        titles = [item['title'] for item in cleaned_data]  # 提取标题
        urls = [item['url'] for item in cleaned_data]  # 提取URL
        ...
        while True:
            try:
                num = int(input("请输入数字（0-50）："))
                if num < 0 or num > 50:
                    print("请输入有效的数字（0-50）！")
                else:
                    print(f"对应标题：{titles[num]}")
                    print(f"对应URL：{urls[num]}")
                    break
            except ValueError:
                print("请输入有效的数字！")
